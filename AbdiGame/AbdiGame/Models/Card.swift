...
enum Card: String, CaseIterable, Identifiable, RawRepresentable {
    ...
    var description: String {
        switch self {
        case .ace:
            return "Players form a sentence, with each player adding one word at a time. The one who fails to keep it up drinks"
        case .king:
            return "You are skipped! Phew..."
        case .queen:
            return "From now on everybody has to begin answering any question with 'Answering your question ...'"
        case .jack:
            return "You select a category for everyone to give an example of. The first one to not give an example is drinks up"
        case .ten:
            return "Create any rule you can imagine that every player (you included) must follow. The game is on until someone fails to follow the rule. The loser drinks up."
        case .nine:
            return "Say something about yourself that is either True or False. The ones who guess it incorrectly drink"
        case .eight:
            return "All players place their fingers on the table, with the last one to do so drinking."
        case .seven:
